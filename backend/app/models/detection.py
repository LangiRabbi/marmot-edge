...
class Detection(BaseModel):
    ...
    # Tracking data
    track_ids: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON
    )  # {"track_ids": [1, 3, 5, ...]}
    tracking_data: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON
    )  # {"tracks": [{"id": 1, "bbox": [...], "conf": 0.95}, ...]}
...
class TrackingSession(BaseModel):
    ...
    zone_history: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON
    )  # {"movements": [{"zone_id": 1, "timestamp": "...", "duration": 30}, ...]}

    # Session stats
    total_detections: Mapped[int] = mapped_column(Integer, default=1)
    zones_visited: Mapped[Optional[Dict[str, Any]]] = mapped_column(
        JSON
    )  # {"zone_ids": [1, 2, 3]}
