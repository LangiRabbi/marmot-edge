...
async def read_workstation(
    ...
    """
    workstation = await workstation_crud.get_workstation(db, workstation_id=workstation_id)
...
async def delete_workstation(
    ...
    """
    success = await workstation_crud.delete_workstation(db=db, workstation_id=workstation_id)
    if not success:
        raise HTTPException(status_code=404, detail="Workstation not found")
