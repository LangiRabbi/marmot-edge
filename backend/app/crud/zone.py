...
async def get_zones(db: AsyncSession, skip: int = 0, limit: int = 100) -> List[Zone]:
    ...
    result = await db.execute(
        select(Zone).offset(skip).limit(limit).order_by(Zone.created_at.desc())
    )
...
async def update_zone(
    db: AsyncSession, zone_id: int, zone_update: ZoneUpdate
) -> Optional[Zone]:
    ...
    if update_data:
        await db.execute(update(Zone).where(Zone.id == zone_id).values(**update_data))
        await db.commit()
        await db.refresh(db_zone)
...
async def delete_zone(db: AsyncSession, zone_id: int) -> bool:
    """Delete a zone"""
    result = await db.execute(delete(Zone).where(Zone.id == zone_id))
    await db.commit()
    return result.rowcount > 0


async def get_zones_count(
    db: AsyncSession, workstation_id: Optional[int] = None
) -> int:
